<#
.SYNOPSIS
    N-Able: Automation Policy - Installation de SQL Server Compact (x64)
.DESCRIPTION
    N-Able: Automation Policy - Installation de SQL Server Compact (x64)
.NOTES
    Fichier    : nable.amp.sql-server-compact.x64.install.ps1
...
try {
    ...
    $IsInstalled = (Test-InstalledSoftware -SoftwareId "{3A9FC03D-C685-4831-94CF-4EDFD3749497}" -SoftwareName "Microsoft SQL Server Compact 3.5 SP2 ENU" -SoftwareVendor "Microsoft Corporation" -SoftwareVersion "3.5.8080.0")

    # install software (32 bit) if not already installed on the current device
    if ($IsInstalled -eq $false) {
        &$choco install sql-server-compact-x86 -y --source "https://chocolatey.cpu.qc.ca/nuget/CPU-CHOCO/" --force
    }
}
catch {
    Send-DataToLogDNA -Message $_ -Severity ERROR -Application "nable.amp.sql-server-compact.x86.install.ps1" -ApiKey "1ec3f3f8a4b8617ec6c8904c9c928a6b"
}

try {
    # check if software (64 bit) is already installed on the current device 
    $IsInstalled = (Test-InstalledSoftware -SoftwareId "{D4AD39AD-091E-4D33-BB2B-59F6FCB8ADC3}" -SoftwareName "Microsoft SQL Server Compact 3.5 SP2 x64 ENU" -SoftwareVendor "Microsoft Corporation" -SoftwareVersion "3.5.8080.0")

    # install software (64 bit) if not already installed on the current device
    if ($IsInstalled -eq $false) {
        &$choco install sql-server-compact-x64 -y --source "https://chocolatey.cpu.qc.ca/nuget/CPU-CHOCO/" --force
    }
}
catch {
    Send-DataToLogDNA -Message $_ -Severity ERROR -Application "nable.amp.sql-server-compact.x64.install.ps1" -ApiKey "1ec3f3f8a4b8617ec6c8904c9c928a6b"
}