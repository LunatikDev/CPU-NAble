<#
.SYNOPSIS
    N-Able: Automation Policy - Installation de SAP Crystal Reports (x64)
.DESCRIPTION
    N-Able: Automation Policy - Installation de SAP Crystal Reports (x64)
.NOTES
    Fichier    : nable.amp.sap-crystal-reports.x64.install.ps1
...
function Test-ChocolateyPending () {
    $IsChocolateyPending =  (Get-ChildItem -Path "C:\ProgramData\chocolatey\lib" -Recurse -Filter ".chocolateyPending")
    if ($null -ne $IsChocolateyPending) {
        $result = $true
    }else{
        $result = $false
    }
...
try {
    # we check if chocolatey is currently running tasks
    $Global:PendingState = Test-ChocolateyPending
    while ($Global:PendingState -eq $true) {
        Start-Sleep -Seconds 3
        $Global:PendingState = Test-ChocolateyPending
    }
    
    # check if software (32 bit) is already installed on the current device 
    $IsInstalled = (Test-InstalledSoftware -SoftwareId "{457B25FC-1E1F-48CA-889C-2ECE37FE1D77}" -SoftwareName "SAP Crystal Reports runtime engine for .NET Framework (32-bit)" -SoftwareVendor "SAP" -SoftwareVersion "13.0.27.3480")
...
try {
    ...
    $IsInstalled = (Test-InstalledSoftware -SoftwareId "{822D6DBD-02BA-4FBC-9C44-F146D9AC4DF3}" -SoftwareName "SAP Crystal Reports runtime engine for .NET Framework (64-bit)" -SoftwareVendor "SAP" -SoftwareVersion "13.0.27.3480")

    # install software (64 bit) if not already installed on the current device
    if ($IsInstalled -eq $false) {
        &$choco install sap-crystal-reports-x64 -y --source "https://chocolatey.cpu.qc.ca/nuget/CPU-CHOCO/" --force
    }
}
catch {
    Send-DataToLogDNA -Message $_ -Severity ERROR -Application "nable.amp.sap-crystal-reports.x64.install.ps1" -ApiKey "1ec3f3f8a4b8617ec6c8904c9c928a6b"
}