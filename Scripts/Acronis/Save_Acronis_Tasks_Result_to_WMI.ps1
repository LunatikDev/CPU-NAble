<#
.SYNOPSIS
    Create Acronis WMI namespace with tasks and alerts classes for monitoring
.DESCRIPTION
    Create Acronis WMI namespace with tasks and alerts classes for monitoring
.NOTES
    Fichier    : Save_Acronis_Tasks_Result_to_WMI.ps1
...
function Get-AcronisTasks { # BUG ??
    param (
        [Parameter(Mandatory=$true,Position=0)]
        [System.Collections.Hashtable]$headers,
        [Parameter(Mandatory=$false,Position=1)]
        [string]$agentID = $AcronisAgentID,
        [Parameter(Mandatory=$false,Position=2)]
        [string]$LastScanTime = ([System.Int64](Get-Date -Date (Get-Date).ToUniversalTime().AddDays(-7).ToString("yyyy-MM-ddT00:00:00Z") -UFormat %s))
    )
    $tasks = Invoke-RestMethod -Uri "https://us-cloud.acronis.com/api/task_manager/v2/tasks?completedAt=gt(${LastScanTime})" -Headers $headers
    $localAgentTasks = ($tasks.items | where {$_.executor.id -eq $agentID})
    $allTasks = @()
    if ($localAgentTasks.count -eq 0){ 
        $allTasks += "No Task Found" 
    }else { 
        foreach ($localAgentTask in $localAgentTasks) {
            $taskObj = new-object -TypeName psobject
            $taskObj | Add-Member -NotePropertyName BackupPlanName -NotePropertyValue ($localAgentTask.policy.name)
            $taskObj | Add-Member -NotePropertyName MachineName -NotePropertyValue ($localAgentTask.resource.name)
            $taskObj | Add-Member -NotePropertyName TaskID -NotePropertyValue ($localAgentTask.idString)
            $taskObj | Add-Member -NotePropertyName TaskType -NotePropertyValue ($localAgentTask.policy.type)
            [string]$sCurrent = $localAgentTask.progress.current
            [string]$sTotal = $localAgentTask.progress.total
            $progress = $sCurrent + " / " + $sTotal
            $taskObj | Add-Member -NotePropertyName TaskProgress -NotePropertyValue ($progress)
            $taskObj | Add-Member -NotePropertyName TaskState -NotePropertyValue ($localAgentTask.state)
            $s = $localAgentTask.startedAt
            $StartDate = $s.Split("T")[0]
            $taskObj | Add-Member -NotePropertyName StartDate -NotePropertyValue ($StartDate)
            $StartTime = ($s.Split("T")[1]).Replace("Z","")
            $taskObj | Add-Member -NotePropertyName StartTime -NotePropertyValue ($StartTime)
            $s = $localAgentTask.completedAt
            $EndDate = $s.Split("T")[0]
            $taskObj | Add-Member -NotePropertyName EndDate -NotePropertyValue ($EndDate)
            $EndTime = ($s.Split("T")[1]).Replace("Z","")
            $taskObj | Add-Member -NotePropertyName EndTime -NotePropertyValue ($EndTime)
            $taskObj | Add-Member -NotePropertyName AgentID -NotePropertyValue ($localAgentTask.executor.id)
            $taskObj | Add-Member -NotePropertyName TenantID -NotePropertyValue ($localAgentTask.tenant.id)
            $taskObj | Add-Member -NotePropertyName TaskUser -NotePropertyValue ($localAgentTask.tenant.name)
            $allTasks += $taskObj
        }
    }
    return $allTasks
}
function Get-AcronisAlerts {
    param (
        [Parameter(Mandatory=$true,Position=0)]
        [System.Collections.Hashtable]$headers,
        [Parameter(Mandatory=$false,Position=1)]
        [string]$resourceName = $AcronisDeviceName,
        [Parameter(Mandatory=$false,Position=2)]
        [string]$LastScanTime = ([System.Int64](Get-Date -Date (Get-Date).ToUniversalTime().AddDays(-7).ToString("yyyy-MM-ddT00:00:00Z") -UFormat %s))
    )
    $localAgentAlerts = Invoke-RestMethod -Uri "https://us-cloud.acronis.com/api/alert_manager/v1/alerts?query=${resourceName}&created_at=gt($LastScanTime)" -Headers $headers
    
    if ($localAgentAlerts.items.count -eq 0){ 
        $allAlerts = "No Active Alerts" 
    }else { 
        $allAlerts = @()
        foreach ($localAgentAlert in $localAgentAlerts.items) {
            $alertObj = new-object -TypeName psobject
            $alertObj | Add-Member -NotePropertyName AlertType -NotePropertyValue ($localAgentAlert.type)
            $alertObj | Add-Member -NotePropertyName ResourceID -NotePropertyValue ($localAgentAlert.details.resourceId)
            $alertObj | Add-Member -NotePropertyName ResourceName -NotePropertyValue ($localAgentAlert.details.resourceName)
            $allAlerts += $alertObj
        }
    }
    return $allAlerts
...
$WMINamespace = "ROOT\ACRONIS"
$tasksClass = "TASKS"
$alertsClass = "ALERTS"
#-----------------------------------------------------------[Execution]------------------------------------------------------------

# authenticate
$headers = (Get-AcronisHeader (Get-AcronisToken))

# get tasks and alerts
$tasks = Get-AcronisTasks $headers
$alerts = Get-AcronisAlerts $headers

# Update timestamp
Update-LastScanTime

#Check if the WMI namespace exists
$isNamepaceExist = Get-WmiNamespace -Namespace $WMINamespace -ErrorAction SilentlyContinue
if ($null -ne $isNamepaceExist) {

    #The namespace already exist, check if the classes exist too
    $isClassExist = Get-WmiClass -Namespace $WMINamespace -ClassName $tasksClass
    if ($null -ne $isClassExist) {

        #remove the old WMI Class
        Remove-WmiClass -Namespace $WMINamespace -ClassName $tasksClass

        #Create WMI tasks Class
        New-WmiClass -Namespace $WMINamespace -ClassName $tasksClass

    }
    $isClassExist = Get-WmiClass -Namespace $WMINamespace -ClassName $alertsClass
    if ($null -ne $isClassExist) {

        #remove the old WMI Class
        Remove-WmiClass -Namespace $WMINamespace -ClassName $alertsClass

        #Create WMI alerts Class
        New-WmiClass -Namespace $WMINamespace -ClassName $alertsClass
    }
}else {
...
#Add the Qualifiers to the WMI Key Classes
[hashtable]$Qualifiers = @{
    Key = $true
    Static = $true
}

#create properties for tasksclass
New-WmiProperty -Namespace $WMINamespace -ClassName $tasksClass -PropertyName 'Name' -PropertyType 'String' -Qualifiers $Qualifiers
New-WmiProperty -Namespace $WMINamespace -ClassName $tasksClass -PropertyName 'BackupPlanName' -PropertyType 'String'
New-WmiProperty -Namespace $WMINamespace -ClassName $tasksClass -PropertyName 'MachineName' -PropertyType 'String'
New-WmiProperty -Namespace $WMINamespace -ClassName $tasksClass -PropertyName 'TaskID' -PropertyType 'String'
New-WmiProperty -Namespace $WMINamespace -ClassName $tasksClass -PropertyName 'TaskType' -PropertyType 'String'
New-WmiProperty -Namespace $WMINamespace -ClassName $tasksClass -PropertyName 'TaskProgress' -PropertyType 'String'
New-WmiProperty -Namespace $WMINamespace -ClassName $tasksClass -PropertyName 'TaskState' -PropertyType 'String'
New-WmiProperty -Namespace $WMINamespace -ClassName $tasksClass -PropertyName 'StartDate' -PropertyType 'String'
New-WmiProperty -Namespace $WMINamespace -ClassName $tasksClass -PropertyName 'StartTime' -PropertyType 'String'
New-WmiProperty -Namespace $WMINamespace -ClassName $tasksClass -PropertyName 'EndDate' -PropertyType 'String'
New-WmiProperty -Namespace $WMINamespace -ClassName $tasksClass -PropertyName 'EndTime' -PropertyType 'String'
New-WmiProperty -Namespace $WMINamespace -ClassName $tasksClass -PropertyName 'AgentID' -PropertyType 'String'
New-WmiProperty -Namespace $WMINamespace -ClassName $tasksClass -PropertyName 'TenantID' -PropertyType 'String'
New-WmiProperty -Namespace $WMINamespace -ClassName $tasksClass -PropertyName 'TaskUser' -PropertyType 'String'
New-WmiProperty -Namespace $WMINamespace -ClassName $tasksClass -PropertyName 'IsFailed' -PropertyType 'Boolean'

foreach ($task in $tasks) {
    if ($task -ne "No Task Found") {
        # Instanciate tasksclasses instances
        [hashtable]$Property = @{
            'Name' = "TASKSCLASS_DATA"
            'BackupPlanName' = $task
            'MachineName' = $task
            'TaskID' = $task
            'TaskType' = $task
        ...
        Write-Host "Task is failed"
    }
}



# Instanciate alertsClass instances
New-WmiProperty -Namespace $WMINamespace -ClassName $alertsClass -PropertyName 'Name' -PropertyType 'String' -Qualifiers $Qualifiers
New-WmiProperty -Namespace $WMINamespace -ClassName $alertsClass -PropertyName 'AlertType' -PropertyType 'String'
New-WmiProperty -Namespace $WMINamespace -ClassName $alertsClass -PropertyName 'ResourceID' -PropertyType 'String'
New-WmiProperty -Namespace $WMINamespace -ClassName $alertsClass -PropertyName 'ResourceName' -PropertyType 'String'
New-WmiProperty -Namespace $WMINamespace -ClassName $alertsClass -PropertyName 'IsFailed' -PropertyType 'Boolean'



#alertsclass values
[hashtable]$Property = @{
    'Name' = "ALERTSCLASS_DATA"
